...
namespace Jvedio.Core.DataBase
{
    public static class Tables
    {
        ...
        public static class Actor
        {
            ...
            static Actor()
            {
                TABLES.Add("actor_info", "BEGIN; create table actor_info( ActorID INTEGER PRIMARY KEY autoincrement, ActorName VARCHAR(500), Country VARCHAR(500), Nation VARCHAR(500), BirthPlace VARCHAR(500), Birthday VARCHAR(100), BloodType VARCHAR(100), Height INT, Weight INT, Gender INT DEFAULT 0, Hobby VARCHAR(500), Cup VARCHAR(1) DEFAULT 'Z', Chest INT, Waist INT, Hipline INT, WebType  VARCHAR(100), WebUrl  VARCHAR(2000), Grade FLOAT DEFAULT 0.0, SmallImagePath TEXT, PreviewImagePath TEXT, ExtraInfo TEXT, CreateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')), UpdateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')) ); CREATE INDEX actor_info_name_idx ON actor_info (ActorName); COMMIT;");
                TABLES.Add("actor_relation", "BEGIN; create table actor_relation( ID INTEGER PRIMARY KEY autoincrement, ActorID INTEGER, ActorIDAnother INTEGER, ActorRelation VARCHAR(500), unique(ActorID,ActorIDAnother,ActorRelation) ); CREATE INDEX actor_relation_idx ON actor_relation (ActorID); COMMIT;");
            }
        }
