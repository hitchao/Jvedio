...
namespace Jvedio.Core.DataBase
{
    public static class Tables
    {
        ...
        public static class Actor
        {
            ...
            static Actor()
            {
                TABLES.Add("actor_info", "BEGIN; create table actor_info( ActorID INTEGER PRIMARY KEY autoincrement, ActorName VARCHAR(500), Country VARCHAR(500), Nation VARCHAR(500), BirthPlace VARCHAR(500), Birthday VARCHAR(100), Age INT, BloodType VARCHAR(100), Height INT, Weight INT, Gender INT DEFAULT 0, Hobby VARCHAR(500), Cup VARCHAR(1) DEFAULT 'Z', Chest INT, Waist INT, Hipline INT, WebType  VARCHAR(100), WebUrl  VARCHAR(2000), Grade FLOAT DEFAULT 0.0, SmallImagePath TEXT, PreviewImagePath TEXT, ExtraInfo TEXT, CreateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')), UpdateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')) ); CREATE INDEX actor_info_name_idx ON actor_info (ActorName); COMMIT;"); TABLES.Add("actor_relation", "BEGIN; create table actor_relation( ID INTEGER PRIMARY KEY autoincrement, ActorID INTEGER, ActorIDAnother INTEGER, ActorRelation VARCHAR(500), unique(ActorID,ActorIDAnother,ActorRelation) ); CREATE INDEX actor_relation_idx ON actor_relation (ActorID); COMMIT;");
            }
        }
        ...
        public static class Data
        {
            ...
            static Data()
            {
                ...
                TABLES.Add("metadata_video", "BEGIN; create table metadata_video( MVID INTEGER PRIMARY KEY autoincrement, DataID INTEGER, VID VARCHAR(500), VideoType INT DEFAULT 0, Series TEXT, Director VARCHAR(100), Studio TEXT, Publisher TEXT, Plot TEXT, Outline TEXT, Duration INT DEFAULT 0, SubSection TEXT, ImageUrls TEXT DEFAULT '', PreviewImagePath TEXT, ScreenShotPath TEXT, GifImagePath TEXT, BigImagePath TEXT, SmallImagePath TEXT, WebType  VARCHAR(100), WebUrl  VARCHAR(2000), ExtraInfo TEXT, unique(DataID,VID) ); CREATE INDEX metadata_video_idx_DataID_VID ON metadata_video (DataID,VID); CREATE INDEX metadata_video_idx_VID ON metadata_video (VID); CREATE INDEX metadata_video_idx_VideoType ON metadata_video (VideoType); COMMIT;");
                ...
                TABLES.Add("metadatas_to_actor", "BEGIN; create table metadatas_to_actor( ID INTEGER PRIMARY KEY autoincrement, ActorID INTEGER, DataID INT, unique(ActorID,DataID) ); CREATE INDEX metadatas_to_actor_idx_ActorID ON metadatas_to_actor (ActorID,DataID); CREATE INDEX metadatas_to_actor_idx_DataID ON metadatas_to_actor (DataID,ActorID); COMMIT;");
                ...
                TABLES.Add("metadata_game", "BEGIN; create table metadata_game( GID INTEGER PRIMARY KEY autoincrement, DataID INTEGER, Branch VARCHAR(100), OriginalPainting VARCHAR(200), VoiceActors VARCHAR(200), Play VARCHAR(200), Music VARCHAR(200), Singers VARCHAR(200), Plot TEXT, Outline TEXT, ExtraName TEXT, Studio TEXT, Publisher TEXT, GifImagePath TEXT, BigImagePath TEXT, SmallImagePath TEXT, WebType  VARCHAR(100), WebUrl  VARCHAR(2000), ExtraInfo TEXT, unique(DataID,GID) ); CREATE INDEX metadata_game_idx_DataID_GID ON metadata_game (DataID,GID); COMMIT;");
            }
        }
