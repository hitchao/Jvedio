...
namespace Jvedio.Core.DataBase
{
    public static class Tables
    {
        ...
        public static class AppData
        {
            ...
            static AppData()
            {
                TABLES.Add("app_databases", "drop table if exists app_databases; BEGIN; create table app_databases ( DBId INTEGER PRIMARY KEY autoincrement, Name VARCHAR(500), Count INTEGER DEFAULT 0, DataType INT DEFAULT 0, ImagePath TEXT DEFAULT '', ViewCount INT DEFAULT 0, CreateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')), UpdateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')) ); CREATE INDEX name_idx ON app_databases (Name); CREATE INDEX type_idx ON app_databases (DataType); COMMIT;");
        ...
        public static class Data
        {
            ...
            static Data()
            {
                ...
                TABLES.Add("metadata_video", "BEGIN; create table metadata_video( DataID INTEGER PRIMARY KEY autoincrement, VID VARCHAR(500), VideoType INT DEFAULT 0, Director VARCHAR(100), Studio TEXT, Publisher TEXT, Plot TEXT, Outline TEXT, Duration INT DEFAULT 0, SubSection TEXT, ImageUrls TEXT DEFAULT '', PreviewImagePaths TEXT, ScreenShotPaths TEXT, GifImagePath TEXT, BigImagePath TEXT, SmallImagePath TEXT, WebType  VARCHAR(100), WebUrl  VARCHAR(2000), ExtraInfo TEXT, unique(DataID,VID) ); CREATE INDEX metadata_video_idx_VID ON metadata_video (VID); CREATE INDEX metadata_video_idx_VideoType ON metadata_video (VideoType); COMMIT;");
                TABLES.Add("metadata_to_translation", "BEGIN; create table metadata_to_translation( id INTEGER PRIMARY KEY autoincrement, DataID INTEGER, FieldType VARCHAR(100), TransaltionID INTEGER ); CREATE INDEX metadata_to_translation_idx_DataID_FieldType ON metadata_to_translation (DataID,FieldType); COMMIT;");
            }
        }
