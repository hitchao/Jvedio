...
namespace Jvedio.Core.Sql
{
    public static class SqliteTables
    {
        ...
        public static class Actor
        {
            ...
            static Actor()
            {
                TABLES.Add("actor_info", "BEGIN; create table actor_info( ActorID INTEGER PRIMARY KEY autoincrement, ActorName VARCHAR(500), NameFlag INT DEFAULT 0, Country VARCHAR(500), Nation VARCHAR(500), BirthPlace VARCHAR(500), Birthday VARCHAR(100), BloodType VARCHAR(100), Height INT, Weight INT, Gender INT DEFAULT 0, Hobby VARCHAR(500), Cup VARCHAR(1), Chest INT, Waist INT, Hipline INT, ExtraInfo TEXT, CreateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')), UpdateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')), unique(ActorName,NameFlag) ); CREATE INDEX actor_info_name_idx ON actor_info (ActorName,NameFlag); COMMIT;");
        ...
        public static class Data
        {
            ...
            static Data()
            {
                TABLES.Add("metadata", "drop table if exists metadata; BEGIN; create table if not exists metadata ( DataID INTEGER PRIMARY KEY autoincrement, Title TEXT, Size  INTEGER DEFAULT 0, Path TEXT, Hash VARCHAR(32), ReleaseDate VARCHAR(30) DEFAULT '1900-01-01', ReleaseYear INT DEFAULT 1900, ViewCount INT DEFAULT 0, DataType INT DEFAULT 0, Rating FLOAT DEFAULT 0.0, RatingCount INT DEFAULT 0, FavoriteCount INT DEFAULT 0, Genre TEXT, Tag TEXT, Grade FLOAT DEFAULT 0.0, Label TEXT, ViewDate VARCHAR(30), CreateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')), UpdateDate VARCHAR(30) DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%S', 'NOW', 'localtime')) ); CREATE INDEX metadata_idx_ReleaseDate ON metadata (ReleaseDate); CREATE INDEX metadata_idx_DataType ON metadata (DataType); CREATE INDEX metadata_idx_Hash ON metadata (Hash); CREATE INDEX metadata_idx_ViewDate ON metadata (ViewDate); COMMIT;");
                TABLES.Add("metadata_video", "BEGIN; create table metadata_video( DataID INTEGER PRIMARY KEY autoincrement, VID VARCHAR(500), VideoType INT DEFAULT 0, Director VARCHAR(100), Country VARCHAR(50), Studio TEXT, Publisher TEXT, Plot TEXT, Outline TEXT, Duration INT DEFAULT 0, PreviewImages TEXT, ExtraInfo TEXT, unique(DataID,VID) ); CREATE INDEX metadata_video_idx_VID ON metadata_video (VID); CREATE INDEX metadata_video_idx_VideoType ON metadata_video (VideoType); COMMIT;");
